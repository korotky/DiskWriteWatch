...
function syncTimeControls(custom){
  $('fromTime').disabled=!custom;
  $('toTime').disabled=!custom;
  $('applyTime').disabled=!custom;
}
function range(){
  if($('range').value==='custom'){
    syncTimeControls(true);
...
function query(){
  currentRange=range();
  const p=new URLSearchParams({from:currentRange.from,to:currentRange.to,includeMonitor:$('includeMonitor').checked});
...
function chart(points){
  ...
  const start=selected.from,end=selected.to;
  ...
  if(!points.length){
    const label='No writes in selected interval';
    const textW=x.measureText(label).width;
    x.fillText(label,Math.max(8,w/2-textW/2),top+plotH/2);
    return;
  }
  const pointX = p => span===0 ? 0 : (p.bucketUnixSeconds-start)/span*w;
  const gapThreshold=Math.max(currentBucketSeconds*2.5, span/200);
  for(let i=1;i<points.length;i++){
    if(points[i].bucketUnixSeconds-points[i-1].bucketUnixSeconds<=gapThreshold)continue;
    const left=pointX(points[i-1]),right=pointX(points[i]);
    x.fillStyle='rgba(145,160,184,.08)';
    x.fillRect(left,top,Math.max(1,right-left),plotH);
    x.strokeStyle='rgba(145,160,184,.35)';x.setLineDash([4,4]);
    x.beginPath();x.moveTo(left,top);x.lineTo(left,top+plotH);x.moveTo(right,top);x.lineTo(right,top+plotH);x.stroke();
    x.setLineDash([]);
  }
  const draw=(key,color)=>{
    x.strokeStyle=color;x.lineWidth=2;x.beginPath();
    points.forEach((p,i)=>{
      const px=pointX(p),py=top+plotH-p[key]/max*(plotH-6);
      if(!i||p.bucketUnixSeconds-points[i-1].bucketUnixSeconds>gapThreshold)x.moveTo(px,py);
      else x.lineTo(px,py);
    });
    x.stroke();
  };
...
async function load(){
  try{
    ...
    currentBucketSeconds=config.bucketSeconds||60;
    ...
    refreshSeconds=config.refreshSeconds||10;$('export').href=`/api/export.csv?${q}`;
...
$('refresh').onclick=load;$('clearFilters').onclick=clearFilters;$('applyTime').onclick=applyTime;$('resetTime').onclick=resetTime;$('resetView').onclick=resetView;$('includeMonitor').onchange=load;
